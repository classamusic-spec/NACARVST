...
/*
    ...
    KNOWN LIMITATIONS
      ...
      - INTEGRATION NOTE, resolved.  NacarEngine calls this engine on every
        block whether or not space_on is true, and applies the FX chain's
        bypass mask by forcing the flag rather than by skipping the call.  The
        engine therefore does see the block in which it was turned off and its
        own flush runs on that transition, on both routes - the power ring and
        MIX reaching zero.  Nothing here depends on the chain gating, and the
        chain must not start.
*/
